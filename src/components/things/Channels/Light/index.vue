...
<script>
  ...
  export default {
    ...
    data() {
      return {
        activeTab: 'white',
        switchProperty: this._.find(this.channel.properties, { 'property': PROPERTY_TYPE_STATE }),
        slider: {
          style: {
            hue: {
    ...
    computed: {
       ...
       */
      switchState() {
        if (this.switchProperty !== undefined) {
          const propertyValue = ChannelPropertyValue
            .query()
            .where('channel_id', this.channel.id)
            .where('property_id', this.switchProperty.id)
            .first()
