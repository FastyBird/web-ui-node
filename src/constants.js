// Channels structures types
export const CHANNEL_TYPE_ANALOG_SENSOR = 'analog_sensor'
export const CHANNEL_TYPE_ANALOG_ACTOR = 'analog_actor'
export const CHANNEL_TYPE_BINARY_SENSOR = 'binary_sensor'
export const CHANNEL_TYPE_BINARY_ACTOR = 'binary_actor'
export const CHANNEL_TYPE_EVENT = 'event'
export const CHANNEL_TYPE_BUTTON = 'button'
export const CHANNEL_TYPE_ENERGY = 'energy'
export const CHANNEL_TYPE_ENVIRONMENT = 'environment'
export const CHANNEL_TYPE_LIGHT = 'light'
export const CHANNEL_TYPE_SWITCH = 'switch'

// Values data types
export const DATA_TYPE_INTEGER = 'integer'
export const DATA_TYPE_FLOAT = 'float'
export const DATA_TYPE_BOOLEAN = 'boolean'
export const DATA_TYPE_STRING = 'string'
export const DATA_TYPE_ENUM = 'enum'
export const DATA_TYPE_COLOR = 'color'

export const PROPERTY_TYPE_STATE = 'state'

export const MANUFACTURER_GENERIC = 'generic'
export const MANUFACTURER_FASTYBIRD = 'fastybird'
export const MANUFACTURER_ITEAD = 'itead'

export const HARDWARE_MODEL_CUSTOM = 'custom'
