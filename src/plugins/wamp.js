...
const WAMP_WS_STATE = 'WAMP_WS_STATE'
const WAMP_RESET_STATE = 'WAMP_RESET_STATE'
const WAMP_VUEX_MODULE = 'wampExchange'
...
function WampV1(host, options) {
  ...
  if (options.store !== undefined) {
    ...
    this.store.registerModule(WAMP_VUEX_MODULE, this.vuex)
  }
}
  ...
  if (!this.isConnected) {
    ...
    this.socket.addEventListener('open', () => {
      ...
      if (this.store !== undefined) {
        this.store.dispatch(`${WAMP_VUEX_MODULE}/setConnecting`, null, {
          root: true,
        })
      }
    })
    ...
    this.socket.addEventListener('close', (event) => {
      ...
      if (this.store !== undefined) {
        this.store.dispatch(`${WAMP_VUEX_MODULE}/setDisconnected`, null, {
          root: true,
        })
      }
    })
    ...
    this.socket.addEventListener('message', (event) => {
      ...
      switch (code) {
        ...
        case 0:
          ...
          if (this.store !== undefined) {
            this.store.dispatch(`${WAMP_VUEX_MODULE}/setConnected`, null, {
              root: true,
            })
          }
          break
...
function plugin(Vue, opt) {
  ...
  const wamp = new WampV1(options.wsuri, {
    debug: options.debug,
    store: options.store,
  })
