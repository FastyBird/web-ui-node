  ...
  >
    ...
    <template slot="form">
      ...
      <fb-md-form-checkboxes-group
        v-if="property !== null && property.isEnum && actions.length"
      ...
      <fb-md-form-checkboxes-group
        v-if="property !== null && property.isBoolean"
        v-model="form.model.operands"
        v-validate="'required'"
        :data-vv-scope="form.scope"
        :error="errors.first(form.scope + '.operands')"
      ...
      <div
        v-if="property !== null && property.isNumber"
        class="row"
      >
        <div class="col-3">
          <fb-md-form-select
    ...
    computed: {
      ...
      thing() {
        return this.$store.getters['entities/thing/query']()
          .with('properties')
          .with('socket')
          .where('id', this.form.model.thing)
          .first()
      },
      ...
      property() {
        if (this.form.model.trigger === 'state') {
          return this.thing ? this.thing.stateProperty : null
        }
      ...
      things() {
        ...
        for (const thing of this.$store.getters['entities/thing/all']()) {
          options.push({
            value: thing.id,
            name: thing.label,
          })
      ...
      triggers() {
        ...
        if (this.form.model.thing !== null) {
          ...
          const thing = this.$store.getters['entities/thing/find'](this.form.model.thing)
      ...
      properties() {
        ...
        if (this.form.model.trigger === 'state') {
          options.push({
            value: thing.stateProperty.id,
            name: this.$tThingProperty(thing, thing.stateProperty),
          })
