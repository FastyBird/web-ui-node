...
import metadata from '@fastybird/node-metadata/resources/schemas/auth-node/entity.email.json'
import { Entity } from '@fastybird/node-metadata/types/auth-node.entity.email'
...
const moduleActions: ActionTree<EmailState, any> = {
  async get({ state, commit }, payload: { account: AccountInterface, id: string }): Promise<boolean> {
    ...
    try {
      await Email.api().get(
        `/auth-node/v1/accounts/${payload.account.id}/emails/${payload.id}`,
        apiOptions,
      )
  ...
  async fetch({ state, commit }, payload: { account: AccountInterface }): Promise<boolean> {
    ...
    try {
      await Email.api().get(
        `/auth-node/v1/accounts/${payload.account.id}/emails`,
        apiOptions,
      )
  ...
  async add({ commit }, payload: { account: AccountInterface, id?: string | null, draft?: boolean, data: EmailCreateInterface }): Promise<Item<Email>> {
    ...
    if (createdEntity === null) {
      ...
      throw new Error('auth-node.emails.create.failed')
    }
  ...
  async edit({ state, commit }, payload: { email: EmailInterface, data: EmailUpdateInterface }): Promise<Item<Email>> {
    if (state.semaphore.updating.includes(payload.email.id)) {
      throw new Error('auth-node.emails.update.inProgress')
    }

    if (!Email.query().where('id', payload.email.id).exists()) {
      throw new Error('auth-node.emails.update.failed')
    }
    ...
    if (updatedEntity.draft) {
      ...
      try {
        await Email.api().patch(
          `/auth-node/v1/accounts/${updatedEntity.accountId}/emails/${updatedEntity.id}`,
          jsonApiFormatter.serialize({
            stuff: updatedEntity,
          }),
          apiOptions,
  ...
  async save({ state, commit }, payload: { email: EmailInterface }): Promise<Item<Email>> {
    if (state.semaphore.updating.includes(payload.email.id)) {
      throw new Error('auth-node.emails.save.inProgress')
    }

    if (!Email.query().where('id', payload.email.id).where('draft', true).exists()) {
      throw new Error('auth-node.emails.save.failed')
    }
    ...
    try {
      await Email.api().post(
        `/auth-node/v1/accounts/${entityToSave.accountId}/emails`,
        jsonApiFormatter.serialize({
          stuff: entityToSave,
        }),
        apiOptions,
  ...
  async remove({ state, commit }, payload: { email: EmailInterface }): Promise<boolean> {
    if (state.semaphore.deleting.includes(payload.email.id)) {
      throw new Error('auth-node.emails.delete.inProgress')
    }

    if (!Email.query().where('id', payload.email.id).exists()) {
      throw new Error('auth-node.emails.delete.failed')
    }
    ...
    if (payload.email.draft) {
      ...
      try {
        await Email.api().delete(
          `/auth-node/v1/accounts/${payload.email.accountId}/emails/${payload.email.id}`,
          {
            save: false,
          },
        )
