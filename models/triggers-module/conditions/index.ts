...
const moduleActions: ActionTree<ConditionState, any> = {
  async get({ state, commit }, payload: { trigger: TriggerInterface, id: string }): Promise<boolean> {
    ...
    try {
      await Condition.api().get(
        `/triggers-module/v1/triggers/${payload.trigger.id}/conditions/${payload.id}`,
        apiOptions,
      )
    ...
    if (draft) {
      ...
      try {
        await Condition.api().post(
          `/triggers-module/v1/triggers/${payload.trigger.id}/conditions`,
    ...
    if (updatedEntity.draft) {
      ...
      try {
        await Condition.api().patch(
          `/triggers-module/v1/triggers/${updatedEntity.triggerId}/conditions/${updatedEntity.id}`,
          jsonApiFormatter.serialize({
            stuff: updatedEntity,
          }),
          apiOptions,
  ...
  async save({ state, commit }, payload: { condition: ConditionInterface }): Promise<Item<Condition>> {
    ...
    try {
      await Condition.api().post(
        `/triggers-module/v1/triggers/${entityToSave.triggerId}/conditions`,
        jsonApiFormatter.serialize({
          stuff: entityToSave,
        }),
        apiOptions,
    ...
    if (payload.condition.draft) {
      ...
      try {
        await Condition.api().delete(
          `/triggers-module/v1/triggers/${payload.condition.triggerId}/conditions/${payload.condition.id}`,
          {
            save: false,
          },
        )
