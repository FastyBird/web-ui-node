...
const moduleActions: ActionTree<NotificationState, any> = {
  async get({ state, commit }, payload: { trigger: TriggerInterface, id: string }): Promise<boolean> {
    ...
    try {
      await Notification.api().get(
        `/triggers-module/v1/triggers/${payload.trigger.id}/notifications/${payload.id}`,
        apiOptions,
      )
    ...
    if (draft) {
      ...
      try {
        await Notification.api().post(
          `/triggers-module/v1/triggers/${payload.trigger.id}/notifications`,
    ...
    if (updatedEntity.draft) {
      ...
      try {
        await Notification.api().patch(
          `/triggers-module/v1/triggers/${updatedEntity.triggerId}/notifications/${updatedEntity.id}`,
          jsonApiFormatter.serialize({
            stuff: updatedEntity,
          }),
          apiOptions,
  ...
  async save({ state, commit }, payload: { notification: NotificationInterface }): Promise<Item<Notification>> {
    ...
    try {
      await Notification.api().patch(
        `/triggers-module/v1/triggers/${entityToSave.triggerId}/notifications`,
        jsonApiFormatter.serialize({
          stuff: entityToSave,
        }),
        apiOptions,
    ...
    if (payload.notification.draft) {
      ...
      try {
        await Notification.api().delete(
          `/triggers-module/v1/triggers/${payload.notification.triggerId}/notifications/${payload.notification.id}`,
          {
            save: false,
          },
        )
