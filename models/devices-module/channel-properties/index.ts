...
import metadata from '@fastybird/modules-metadata/resources/schemas/devices-module/entity.device.property.json'
import { Entity } from '@fastybird/modules-metadata/types/devices-module.entity.device.property'
...
const moduleActions: ActionTree<ChannelPropertyState, any> = {
  async get({ state, commit }, payload: { channel: ChannelInterface, id: string }): Promise<boolean> {
    ...
    try {
      await ChannelProperty.api().get(
        `/devices-module/v1/devices/${payload.channel.deviceId}/channels/${payload.channel.id}/properties/${payload.id}`,
        apiOptions,
      )
  ...
  async fetch({ state, commit }, payload: { channel: ChannelInterface }): Promise<boolean> {
    ...
    try {
      await ChannelProperty.api().get(
        `/devices-module/v1/devices/${payload.channel.deviceId}/channels/${payload.channel.id}/properties`,
        apiOptions,
      )
  ...
  async edit({ state, commit }, payload: { property: ChannelPropertyInterface, name?: string | null, unit?: string, format?: string, expected?: any }): Promise<Item<ChannelProperty>> {
    ...
    try {
      await ChannelProperty.api().patch(
        `/devices-module/v1/devices/${channel.deviceId}/channels/${updatedEntity.channelId}/properties/${updatedEntity.id}`,
