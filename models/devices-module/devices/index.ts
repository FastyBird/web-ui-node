...
import metadata from '@fastybird/modules-metadata/resources/schemas/devices-module/entity.device.json'
import {
  Entity,
  Type,
} from '@fastybird/modules-metadata/types/devices-module.entity.device'
...
const moduleActions: ActionTree<DeviceState, any> = {
  async get({ state, commit }, payload: { id: string, includeChannels: boolean }): Promise<boolean> {
    ...
    try {
      await Device.api().get(
        `/devices-module/v1/devices/${payload.id}?include=hardware,firmware,properties,configuration`,
        apiOptions,
      )
  ...
  async fetch({ state, commit }, payload: { includeChannels: boolean }): Promise<boolean> {
    ...
    try {
      await Device.api().get(
        '/devices-module/v1/devices?include=hardware,firmware,properties,configuration',
        apiOptions,
      )
    ...
    if (updatedEntity.draft) {
      ...
      try {
        await Device.api().patch(
          `/devices-module/v1/devices/${updatedEntity.id}?include=hardware,firmware,properties,configuration`,
          jsonApiFormatter.serialize({
            stuff: updatedEntity,
          }),
          apiOptions,
    ...
    if (payload.device.draft) {
      ...
      try {
        await Device.api().delete(
          `/devices-module/v1/devices/${payload.device.id}`,
          {
            save: false,
          },
        )
