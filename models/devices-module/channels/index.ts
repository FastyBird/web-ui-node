...
import metadata from '@fastybird/modules-metadata/resources/schemas/devices-module/entity.channel.json'
import { Entity } from '@fastybird/modules-metadata/types/devices-module.entity.channel'
...
const moduleActions: ActionTree<ChannelState, any> = {
  async get({ state, commit }, payload: { device: DeviceInterface, id: string }): Promise<boolean> {
    ...
    try {
      await Channel.api().get(
        `/devices-module/v1/devices/${payload.device.id}/channels/${payload.id}?include=properties,configuration`,
        apiOptions,
      )
  ...
  async fetch({ state, commit }, payload: { device: DeviceInterface }): Promise<boolean> {
    ...
    try {
      await Channel.api().get(
        `/devices-module/v1/devices/${payload.device.id}/channels?include=properties,configuration`,
        apiOptions,
      )
  ...
  async edit({ state, commit }, payload: { channel: ChannelInterface, data: ChannelUpdateInterface }): Promise<Item<Channel>> {
    ...
    try {
      await Channel.api().patch(
        `/devices-module/v1/devices/${updatedEntity.deviceId}/channels/${updatedEntity.id}?include=properties,configuration`,
